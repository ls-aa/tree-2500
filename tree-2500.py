...
def generate_tree2500_5(
    ...
    # 3. 每个社区内部加少量长距离边，避免三角形，保持 tree-like
    for c, nodes in enumerate(communities):
        added = 0
        tries = 0
    ...
    np.savez(
        os.path.join(out_dir, "split.npz"),
        train_idx=train_idx,
        val_idx=val_idx,
        test_idx=test_idx,
